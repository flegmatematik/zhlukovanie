...
cluster_centerX = boxX/2.0
cluster_centerY = cell_radius * 5
cluster_centerZ = boxZ/2.0

x = cluster_centerX
y = cluster_centerY
z = cluster_centerZ
d = 2 * cell_radius + gap_size
...
for i in range(1,cell_count):
    ...
    while not allOk:
        ...
        #zatocenie - pitch
        #naklonenie - yaw
        temp_zatocenie = zatocenie + randint(-max_angle, max_angle)
        temp_naklonenie = naklonenie + randint(-max_angle, max_angle)

        ang_x = math.sin(math.radians(temp_naklonenie)) * math.cos(math.radians(temp_zatocenie))
        ang_y = math.sin(math.radians(temp_zatocenie))
        ang_z = math.cos(math.radians(temp_naklonenie)) * math.cos(math.radians(temp_zatocenie))
        ...
        for cell_center in cell_positions:
            if distance(cell_center,[new_x,new_y,new_z]) < d:
                allOk = False
    ...
    zatocenie = temp_zatocenie
    naklonenie = temp_naklonenie
